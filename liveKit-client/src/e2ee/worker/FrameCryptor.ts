...
import type { DecodeRatchetOptions, KeyProviderOptions, KeySet } from 'livekit-client'
import type { CryptorCallbacks } from '../errors'
import { CryptorEvent } from 'livekit-client'
import { deriveKeys, isVideoFrame } from 'livekit-client'
...
export class FrameCryptor extends BaseFrameCryptor {
	...
	constructor(opts: {
		...
		this.unencryptedFrameByteTrailer = opts.unencryptedFrameBytes ?? new TextEncoder().encode('LKROCKS')
	}
	...
	setupTransform(operation: 'encode' | 'decode', readable: ReadableStream, writable: WritableStream, trackId: string, codec?: VideoCodec) {
		if (codec) {
	 ...
	 */
	async encodeFunction(encodedFrame: RTCEncodedVideoFrame | RTCEncodedAudioFrame, controller: TransformStreamDefaultController) {
		if (
			!this.keys.isEnabled() ||
			...
			try {
				...
				if (error instanceof CryptorError && error.reason === CryptorErrorReason.InvalidKey) {
					if (this.keys.hasValidKey) {
						...
						this.emit(CryptorEvent.Error, new CryptorError(`invalid key for participant ${this.participantId}`, CryptorErrorReason.InvalidKey))
	...
	async decryptFrame(
		...
		try {
			...
			if (this.keyProviderOptions.ratchetWindowSize > 0) {
				...
				this.emit(CryptorEvent.Error, new CryptorError(`valid key missing for participant ${this.participantId}`, CryptorErrorReason.MissingKey))
			} else {
	...
	getUnencryptedBytes(frame: RTCEncodedVideoFrame | RTCEncodedAudioFrame): number {
		if (isVideoFrame(frame)) {
			...
			try {
				...
				const isH264 =
					detectedCodec === 'h264' ||
					naluIndices.some((naluIndex) => [NALUType.SLICE_IDR, NALUType.SLICE_NON_IDR].includes(parseNALUType(data[naluIndex])))
